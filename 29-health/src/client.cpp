...
public:
	explicit SumClient(std::shared_ptr<grpc::Channel> channel) 
		: stub(sum::SumService::NewStub(channel)), health_stub(Health::NewStub(channel))
	{
	}
	...
	bool HealthCheck()
	{
    	...
    	grpc::Status status = this->health_stub->Check(&context, HealthCheckRequest::default_instance(), &response);
    	if (!status.ok())
		{
     		std::cerr << "Failed to check service health: " << status.error_code() << ": " << status.error_message() << std::endl;
      		return false;
    	}

    	std::cout << response.DebugString();

		return (response.status() == grpc::health::v1::HealthCheckResponse_ServingStatus_SERVING);
  }

private:
	std::unique_ptr<sum::SumService::Stub> stub;
	std::unique_ptr<Health::Stub> health_stub;
};
...
int main(int argc, char** argv) 
{
	...
	{
		std::cerr << "Could not load service policy. ERROR: " << e.what() << std::endl;
		return 1000;
	}
